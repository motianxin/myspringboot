...
public class LambdaTest {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Integer> name_scoreMap =
                students.stream().filter(student -> student.getScore() > 40).collect(Collectors.toMap(Student::getName, Student::getScore, (oldValue, value) -> value));
        ...
        Map<String, List<String>> map = students.stream().collect(Collectors.groupingBy(Student::getGrade,
                Collectors.mapping(Student::getName, Collectors.toList())));
        System.out.println(map);

        System.out.println("找出一个年级有多少人");
        Map<String, Long> grade_personNumMap = students.stream().collect(Collectors.groupingBy(Student::getGrade,
                Collectors.counting()));
        System.out.println(grade_personNumMap);

        System.out.println("找到一个年级对应分数最高分的学生：");
        Map<String, Student> gradeMaxScoreStuMap = students.stream().collect(Collectors.groupingBy(Student::getGrade,
        ...
        Map<String, List<Student>> grade_sortStuMap =
        ...
        Map<String, List<Student>> grade_lowStuMap =
                students.stream().collect(Collectors.groupingBy(Student::getGrade,
    ...
    private static <T> Collector<T, ?, List<T>> collectingAndSort(Collector<T, ?, List<T>> downstream, Comparator<?
            super T> comparator) {
