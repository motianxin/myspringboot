...
public class AssignmentProblem {
        ...
        for (int i = 0; i < this.n; i++) {
            for (int j = 0; j < this.n; j++) {
                if (this.xy[i] == j) {
                    ...
                    G.addEdge(new DirectedEdge(i, this.n + j, reducedCost(i, j)));
                }
            }
        }
    ...
    private double reducedCost(int i, int j) {
        ...
        if (Math.abs(reducedCost) <= AssignmentProblem.FLOATING_POINT_EPSILON * magnitude) {
            return 0.0;
        }
    ...
    private boolean isComplementarySlack() {
        ...
        for (int i = 0; i < this.n; i++) {
            if ((this.xy[i] != AssignmentProblem.UNMATCHED) && (reducedCost(i, this.xy[i]) != 0)) {
